...
use warnings;
use base
    qw( MT::ArchiveType::ContentTypeAuthor MT::ArchiveType::ContentTypeAuthorYearly FiscalYearlyArchives::AuthorFiscalYearly );

use MT::ContentStatus;
use FiscalYearlyArchives::Util qw( start_end_fiscal_year );

sub name {
    return 'ContentType-Author-Fiscal-Yearly';
}

sub archive_label {
    my $plugin = MT->component("FiscalYearlyArchives");
    return $plugin->translate("CONTENTTYPE-AUTHOR-FISCAL-YEARLY_ADV");
}
...
sub default_archive_templates {
    return [
        {   label           => 'author/author-basename/fy/yyyy/index.html',
            template        => 'author/%-a/fy/%y/%f',
            default         => 1,
        ...
        {   label           => 'author/author_basename/fy/yyyy/index.html',
            template        => 'author/%a/fy/%y/%f',
            required_fields => { date_and_time => 1 }
        },
        {   label           => 'author/author_basename/fy_yyyy/index.html',
            template        => 'author/%a/fy_%y/%f',
            required_fields => { date_and_time => 1 }
        },
    ];
}
...
sub archive_group_iter {
    ...
    my $loop_sub = sub {
        ...
        my $group_terms
            = $obj->make_archive_group_terms( $blog->id, $dt_field_id, $ts, $tsend,
    ...
    return sub {
        if ( $curr < $loop ) {
            my $date
                = sprintf( "%04d%02d%02d000000", $data[$curr]->{year}, $data[$curr]->{month}, 1 );
...
*archive_file  = \&FiscalYearlyArchives::AuthorFiscalYearly::archive_file;
*archive_title = \&FiscalYearlyArchives::AuthorFiscalYearly::archive_title;
