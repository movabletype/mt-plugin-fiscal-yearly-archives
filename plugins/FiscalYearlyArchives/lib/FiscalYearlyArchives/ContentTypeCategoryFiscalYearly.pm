...
use warnings;
use base
    qw( MT::ArchiveType::ContentTypeCategory MT::ArchiveType::ContentTypeYearly FiscalYearlyArchives::CategoryFiscalYearly );

use FiscalYearlyArchives::Util qw( start_end_fiscal_year );

sub name {
    return 'ContentType-Category-Fiscal-Yearly';
}

sub archive_label {
    my $plugin = MT->component("FiscalYearlyArchives");
    return $plugin->translate("CONTENTTYPE-CATEGORY-FISCAL-YEARLY_ADV");
}
...
sub default_archive_templates {
    return [
        {   label           => 'category/sub-category/fy/yyyy/index.html',
            template        => '%-c/fy/%y/%i',
        ...
        {   label           => 'category/sub_category/fy/yyyy/index.html',
            template        => '%c/fy/%y/%i',
            required_fields => { category => 1, date_and_time => 1 }
        },
        {   label           => 'category/sub_category/fy_yyyy/index.html',
            template        => '%c/fy_%y/%i',
            required_fields => { category => 1, date_and_time => 1 }
        },
    ];
...
*archive_title = \&FiscalYearlyArchives::CategoryFiscalYearly::archive_title;
